...
from core_explore_common_app.constants import LOCAL_QUERY_URL
...
class TypeIndexView(IndexView):
    def __init__(self, **kwargs):
        ...
        self.list_complex_type_ids = [
            str(complex_type.id) for complex_type in type_api.get_all_complex_type()
        ]
    ...
    def get_global_active_list(self):
        ...
        global_active_list = global_active_list.filter(
            current__in=self.list_complex_type_ids
        )
    ...
    def get_user_active_list(self, user_id):
        ...
        # Filter only complex_type
        user_active_list = user_active_list.filter(
            current__in=self.list_complex_type_ids
        )
...
class TypeBuildQueryView(BuildQueryView):
    ...
    def _get_css():
        ...
        # Add custom css
        base_css.extend(
            ["core_explore_example_type_app/user/css/query_builder_type.css",]
        )
        return base_css
