    ...
    def render(self, partial=False, full_path=""):
        ...
        if partial:
            ...
            if element_id is not None:
                prefix = data_structure_type_element_api.get_by_data_structure_id(element_id).path
        ...
        for child_key in child_keys:
            ...
            for child in children[child_key]:
                if child.tag == 'element':
                    ...
                    sub_elements.append(self.render_sequence(element=child, full_path=full_path))
                elif child.tag == 'choice':
        ...
        for iter_element in child_keys:
            ...
            for child in children[iter_element]:
                ...
                if child.tag == 'element':
                    ...
                    options.append((str(child.pk), 'Sequence '+str(item_number),
                                    is_selected_element))
                    item_number += 1

                    element_html = self.render_sequence(element=child, full_path=full_path)
                elif child.tag == 'simple_type':
                    options.append((str(child.pk), child.options['name'], is_selected_element))
                ...
                if element_html != '':
                    sub_content += self._render_ul(element_html, str(child.pk),
                                                   (not is_selected_element))
            ...
            if children_number == 0:  # Choice has no child
                ...
                if len(children[iter_element]) == 1:
                    ...
                    html_content += 'Choice ' + self._render_select(None, 'choice', options) + \
                                    buttons
...
def _build_full_path(full_path, is_complex_or_simple_type, element):
   ...
   """
    ...
    if is_complex_or_simple_type and 'type' in element.options and element.options['type'] is \
            not None:
...
def _create_data_structure_type_element(full_path, data_structure):
    ...
    """
    try:
        data_structure_type = DataStructureTypeElement(path=full_path,
